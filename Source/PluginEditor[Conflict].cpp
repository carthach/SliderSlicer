    ...
    inSliceSubDivSlider(Slider::Rotary, Slider::NoTextBox), outSliceSubDivSlider(Slider::Rotary, Slider::NoTextBox)
{
    ...
    inLabel.setBounds (15, 50, 400, 25);
    inLabel.setText("In", NotificationType::dontSendNotification);
    inLabel.setFont(Font("Verdana", 25.0, 0));
    ...
    inSliceSubDivSlider.setBounds (inLabel.getX(), inLabel.getY()+25, 40, 40);
    
    inSliceSubDivSlider.setValue(3.0);
    ...
    outLabel.setBounds (15, 175, 400, 25);
    outLabel.setText("Out", NotificationType::dontSendNotification);
    outLabel.setFont(Font("Verdana", 25.0, 0));
    
    addAndMakeVisible (outSliceSubDivSlider);
    outSliceSubDivSlider.addListener (this);
    outSliceSubDivSlider.setRange (2.0, 5.0, 1.0);
    outSliceSubDivSlider.setValue(2.0);
    outSliceSubDivSlider.setBounds (15, outLabel.getY()+25, 40, 40);
    
    outSliceSubDivSlider.setValue(3.0);
    
    addAndMakeVisible(outSliceSubDivLabel);
    outSliceSubDivLabel.setJustificationType(Justification::centred);
    outSliceSubDivLabel.setBounds(outLabel.getX()-5, outLabel.getY()+22, 50, 50);
    
    //Buttons
    addAndMakeVisible(binaryToggleButton);
    binaryToggleButton.setBounds(500, inSliceSubDivSlider.getY(), 100, 30);
    binaryToggleButton.addListener(this);
    binaryToggleButton.setButtonText("Binary");
    
    addAndMakeVisible(randomButton);
    randomButton.setBounds(500, inSliceSubDivSlider.getY()+50, 100, 25);
    randomButton.addListener(this);
    randomButton.setButtonText("Randomise");
    
    
    //Sliders
    inSliceSliders = setupSliders(inSliceSubDiv, inLabel.getX()+50, inLabel.getY(), false);
    inSliceOverlaySliders = setupSliders(inSliceSubDiv, inLabel.getX()+50, inLabel.getY(), true);
    
    outSliceSliders = setupSliders(outSliceSubDiv, outLabel.getX()+50, outLabel.getY(),false);
    outSliceOverlaySliders = setupSliders(outSliceSubDiv, outLabel.getX()+50, outLabel.getY(), true);
}
...
void SlicerAudioProcessorEditor::sliderValueChanged (Slider* slider)
{
    if (slider == &inSliceSubDivSlider) {
        inSliceSubDiv = 1 << (int)slider->getValue();
    ...
    if (slider == &outSliceSubDivSlider) {
        outSliceSubDiv = 1 << (int)slider->getValue();
...
{
    Colour backgroundColour = Colours::darkcyan;
    
    g.setGradientFill (ColourGradient (backgroundColour, 0.0f, 0.0f,
                                       backgroundColour.darker (0.1f), 0.0f, getHeight(),
                                       false));
